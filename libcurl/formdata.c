    ...
    if(type != FORM_FILE)
      ...
      *size += length;
      ...
      if(strcmp("-", newform->line)) {
        struct_stat file;
        if(!_new_stat(newform->line, &file) && !S_ISDIR(file.st_mode))
    ...
    do {
      ...
      if((post->flags & HTTPPOST_FILENAME) ||
        ...
        fileread = !strcmp("-", file->contents)?
          stdin:_new_fopen(file->contents, "rb"); /* binary read for win32  */
